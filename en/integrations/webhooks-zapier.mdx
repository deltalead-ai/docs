...
| `message.received` | A new inbound message arrives from a lead |
...
<Steps>
  <Step title="Open Webhook settings">
    In DeltaLead, go to **Organization → Integrations → Custom Webhooks**.
  </Step>
...
<Card title="REST API Reference" icon="code" href="/en/api-reference/introduction">
  Authenticate with an `Authorization: Bearer dl_live_...` key and access all DeltaLead resources at `https://platform-api.deltalead.ai/v1`. Full endpoint reference, request/response schemas, and code examples available.
</Card>
