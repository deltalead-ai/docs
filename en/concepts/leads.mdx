...
<Note>
  Export formats include CSV and JSON for lead records and conversation data, available on request from support, or programmatically via `GET /leads` and `GET /conversations` on the DeltaLead API (`https://platform-api.deltalead.ai/v1`). Call recordings are exported as MP3 files.
</Note>
