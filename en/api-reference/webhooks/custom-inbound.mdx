...
<ParamField body="lead.vehicle_of_interest" type="string">
  Free-text description of the vehicle the lead asked about, e.g. `Toyota Hilux SRX 2024`. Resolved against your catalogue while the lead is processed. A match is added to the lead's `interested_vehicles`; with no match the value is kept as a note for an advisor to resolve by hand. Send what the buyer actually wrote rather than an internal stock code. Truncated at 200 characters.
</ParamField>
...
<ParamField body="lead.metadata" type="object">
  Arbitrary JSON object stored on the lead as passthrough, merged across deliveries rather than overwritten. Use it for identifiers from your own system that have no home in the contract. Nothing in DeltaLead interprets it and it is never shown to the AI agent. Keys that start with `$` or contain `.` are silently discarded; the rest of the object is stored and the request returns `200`.
</ParamField>
...
A delivery carrying a key already seen for that account and mode returns `200 {"status": "duplicate"}` and creates nothing.

Idempotency keys are scoped to the **(account, mode)** pair. The two modes keep separate ledgers, so a value burned with a sandbox key does not suppress the same value arriving later with a live key — test values are safe to carry over when you go live.
...
Rate limits apply per route. Separately, the contract fields carry their own content limits:

| Field | Limit | On excess |
|---|---|---|
| `lead.notes` | 4,000 characters | truncated |
| `lead.vehicle_of_interest` | 200 characters | truncated |
| `metadata` — nesting depth | 4 levels | the branch is dropped |
| `metadata` — key length | 128 characters | truncated |
| `metadata` — string value | 2,048 characters | truncated |
| `metadata` — items per array | 50 | the remainder is dropped |
| `metadata` — total size | 8 KB | keys are dropped from the end |

Nothing in the response reports truncation, so keep values inside these bounds rather than relying on the platform to tell you.
...
| `200` | `duplicate` | The `Idempotency-Key` has already been seen for that account and mode |
