...
<Note>
  Export formats include CSV and JSON for lead records and conversation data, accessible from the dashboard under **Settings → Data Export** or programmatically via `GET /leads` and `GET /conversations` on the DeltaLead API (`https://api.deltalead.ai/v1`). Call recordings are exported as MP3 files.
</Note>
