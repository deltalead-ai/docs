...
El Custom Webhook recibe un lead enviado a DeltaLead desde un sistema externo. Una sola ruta, un solo propósito, una sola cabecera obligatoria. Para crear claves desde el panel y entender la diferencia entre modos, ver la [guía del Custom Webhook](/es/integrations/custom-webhook).
...
<Warning>
  Estos cuatro campos son todo el contrato. Cualquier otra clave dentro de `lead` se acepta —la solicitud igual devuelve `200`— pero se **descarta** y nunca llega al registro del lead. No hay que contar con que las claves extra sobrevivan a la solicitud.
</Warning>
