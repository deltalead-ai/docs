...
description: "DeltaLead list endpoints use cursor-based pagination. Supply the cursor and limit parameters to step through leads, conversations, and agent records."
---
...
<Tip>
  Set `limit=100` when you need to fetch a large number of records. Using the maximum page size minimises the total number of HTTP round-trips required, which reduces both latency and the risk of hitting rate limits. See [Rate Limits](/api-reference/introduction#rate-limits) for details.
</Tip>
