...
<ParamField body="lead.vehicle_of_interest" type="string">
  Descripción libre del vehículo por el que consulta el lead, por ejemplo `Toyota Hilux SRX 2024`. Se resuelve contra el catálogo al procesar el lead. Si hay coincidencia, se agrega a `interested_vehicles` del lead; si no la hay, el valor queda como nota para que un vendedor la resuelva a mano. Conviene enviar lo que escribió el comprador y no un código interno de stock. Se trunca a 200 caracteres.
</ParamField>
...
Un envío que trae una clave ya vista para esa cuenta y ese modo devuelve `200 {"status": "duplicate"}` y no crea nada.

Las claves de idempotencia tienen alcance de **(cuenta, modo)**. Los dos modos llevan cuentas separadas, así que un valor ya usado con una clave sandbox no suprime ese mismo valor cuando después llega con una clave live: los valores de prueba se pueden arrastrar sin riesgo al pasar a producción.
...
Los límites de tasa se aplican por ruta. Aparte, los campos del contrato tienen sus propios límites de contenido:

| Campo | Límite | Al excederse |
|---|---|---|
| `lead.notes` | 4.000 caracteres | se trunca |
| `lead.vehicle_of_interest` | 200 caracteres | se trunca |
| `metadata` — profundidad | 4 niveles | se descarta la rama |
| `metadata` — largo de clave | 128 caracteres | se trunca |
| `metadata` — valor de texto | 2.048 caracteres | se trunca |
| `metadata` — items por arreglo | 50 | se descarta el resto |
| `metadata` — tamaño total | 8 KB | se descartan claves desde el final |

La respuesta no informa los truncamientos, así que conviene mantener los valores dentro de estos límites en lugar de esperar que la plataforma avise.
...
| `200` | `duplicate` | La `Idempotency-Key` ya se había visto para esa cuenta y ese modo |
