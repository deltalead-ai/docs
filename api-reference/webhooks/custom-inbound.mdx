...
<Warning>
  Deliveries sent **without** an `Idempotency-Key` are not deduplicated. Two identical requests create two leads. If your client retries — on a timeout, a `500`, or a network error — send an `Idempotency-Key` or you will duplicate the lead.
</Warning>
