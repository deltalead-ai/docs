...
Without the header, DeltaLead falls back to a content hash of the payload, so an identical retry is still deduplicated. Sending an explicit key is preferred: it lets you retry a delivery whose payload is genuinely identical to an earlier, unrelated one without losing the second lead.
...
| Status | Body | When |
|---|---|---|
| `200` | `{"status":"ok"}` | Delivery accepted for processing |
| `200` | `{"status":"duplicate"}` | The `Idempotency-Key` has already been seen |
| `400` | `invalid_body` | The body could not be decoded |
| `400` | `invalid_json` | The body decoded but is not a JSON object |
| `400` | `missing_required_fields` | Lead contract v1 is not satisfied |
...
<Tip>
  Retry on `500 enqueue_failed` — the delivery was valid and the failure is on our side. Do not retry any `4xx`; the request will fail again unchanged. Retries are always safe when you send an `Idempotency-Key`.
</Tip>
