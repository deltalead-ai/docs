...
The Custom Webhook is how an external system pushes a lead **into** DeltaLead. Your landing page, your dealer management system, a partner's lead feed, or an internal script sends one authenticated `POST` request, and the lead lands in your account. This is the inbound direction — the opposite of [Webhooks & Zapier](/integrations/webhooks-zapier), which sends events from DeltaLead out to your endpoint.
...
<Steps>
  ...
  <Step title="Read the response">
    ...
    Any other status means the delivery was not accepted — see the [full response reference](/api-reference/webhooks/custom-inbound#responses).
  </Step>
...
<CardGroup cols={2}>
  <Card title="Endpoint Reference" icon="code" href="/api-reference/webhooks/custom-inbound">
    The full lead contract, idempotency, rate limits, and every response code.
  </Card>
  <Card title="Outbound Webhooks" icon="webhook" href="/integrations/webhooks-zapier">
